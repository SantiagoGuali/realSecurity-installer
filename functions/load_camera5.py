...
UMBRAL_REFLEJO = 5                       # Se reduce de 10 a 5 para descartar imágenes sin detalle (fotografías)
...
THRESHOLD = 15                           # Mínima variación (en la medida que se utilice) para considerar movimiento
...
def es_suplantacion(rostro, frame):
    # Si la imagen tiene muy pocos detalles o no se detecta parpadeo, se asume foto
...
# --- Función para validar micromovimientos (usando desviación estándar) ---
def validar_micro_movimientos(historial_posiciones, threshold):
    if len(historial_posiciones) < 2:
        return False
    xs = np.array([pos[0] for pos in historial_posiciones])
    ys = np.array([pos[1] for pos in historial_posiciones])
    std_x = np.std(xs)
    std_y = np.std(ys)
    # Se requiere que al menos en una dirección la variabilidad supere el umbral
    return (std_x >= threshold) or (std_y >= threshold)
