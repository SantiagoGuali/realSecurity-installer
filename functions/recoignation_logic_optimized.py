...
def procesar_rostro(frame, x1, y1, x2, y2, embeddings_bd, nombres_empleados, umbral_confianza=FAIL_PERCENT,
                    identidad_actual="Desconocido", confianza_actual=0, ultimo_embedding=None, identidad_confirmada=False):
...
def reconocimiento_facial_optimizado():
    cap = cv2.VideoCapture(0)  # En Windows, usar DirectShow
    if not cap.isOpened():
        print("No se pudo abrir la cámara")
        return

    # Instanciar FaceMesh y FaceDetection
        ...
        while True:
            ...
            if mesh_results.multi_face_landmarks:
                for face_landmarks in mesh_results.multi_face_landmarks:
                    # Parpadeo
                    parpadeo = detectar_parpadeo(frame, face_landmarks)
                    # Reflejos
                    reflejo = detectar_reflejos(frame)
                    # Movimiento ojos
                    mov_ojos = detectar_movimiento_ojos(face_landmarks.landmark)
            ...
            # Enviamos a procesar en paralelo
            for face in faces:
            ...
            for future in concurrent.futures.as_completed(future_to_bbox):
                ...
                try:
                    ...
                    identidad_actual = nombre_asignado
                    confianza_actual = conf_asignada
                    ultimo_embedding = emb_detectado
                    identidad_confirmada = id_confirm
...
reconocimiento_facial_optimizado()
